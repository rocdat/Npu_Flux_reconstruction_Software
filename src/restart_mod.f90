...
subroutine check_restart_dir
  ...
  logical(ldk) :: load_restart_file_temp
  !
  !.. Local Parameters ..
...
continue
  ...
  if (mypnum == glb_root) then
    ...
    ! To be compiled on Tianhe-2, the assignment should be like this.
    ! This is weird! Not sure why.
    load_restart_file_temp = logical(load_restart_file,kind=ldk)
    restart_logicals(:) = [ load_restart_file_temp , &
