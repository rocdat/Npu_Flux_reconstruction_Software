...
continue
  ...
  ! NOTE: The inquire function is differently defined in gfortran and intel.
  !       But using like this should be OK for both compilers.
  !
...
continue
  ...
  if (.not. allocated(grid)) then
    ! Drop the support for linear format. The design of BC is bad. Forcus on
    ! GMSH format. 2019-04-19, 13:43:54. Jingchang Shi
    call stop_gfr(stop_mpi,pname,__LINE__,__FILE__, &
      'Linear grid format is not supported for now!')
...
!###############################################################################
!
pure function reorder_linear_pts(elem,reverse) result(return_value)
  !
  !.. Use Statements ..
